<%@ Page Title="" Language="C#" MasterPageFile="~/content/content.master" AutoEventWireup="true" CodeFile="sy_ms_content.aspx.cs" Inherits="content_sy_ms_content" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
   ...
   <div class="l_top">
       ...
       <div class="t_right">当前的位置：<a href="sy_meishi.aspx">首页</a>&nbsp;>&nbsp;美食内容</div>
   </div>
   <div class="l_con1">
       <asp:Repeater ID="rptgy" runat="server" OnItemDataBound="rptgy_ItemDataBound">
        ...
        <ItemTemplate>
           <li class="ltop">
               <asp:Label ID="lbtitle" runat="server" Text='<%# Eval("msTitle") %>'></asp:Label><li>
           <li class="ledit">
               编辑：<asp:Label ID="lbedit" runat="server" Text='<%# Eval("msfbman") %>'></asp:Label></li>
           <li class="ltime">
               编辑时间：<asp:Label ID="lbtime" runat="server" Text='<%# Eval("msTime") %>'></asp:Label></li>
           <li class="limg">
               <asp:Image ID="imgxy" runat="server" ImageUrl='<%# Eval("msImg") %>' />
               <span class="imgzt"><asp:Label
                   ID="lbimg" runat="server" Text=""></asp:Label></span>
           </li>
           <li class="lcon">
               <asp:Literal ID="ltcon" runat="server" Text='<%# Eval("msCon") %>'></asp:Literal></li>
           <li class="lpl">评论（<asp:Label ID="lbplnum" runat="server" Text='<%# Eval("mspl") %>'></asp:Label>）</li>
        </ItemTemplate>
   ...
   <div class="wz">
       <div class="wzxs">
           <asp:Repeater ID="rptgs" runat="server">
               <HeaderTemplate><ul></HeaderTemplate>
               <ItemTemplate>
                   <li><span class="zitixs">上一篇</span>：<asp:HyperLink ID="hylss" runat="server" Text='<%# ziduanjq.Jq_xwbtxs(Eval("msTitle").ToString()) %>' NavigateUrl='<%# Eval("msId","sy_ms_content.aspx?ms_id={0}") %>'>
                       </asp:HyperLink><br/></li>
               </ItemTemplate>
               <FooterTemplate></ul></FooterTemplate>
           </asp:Repeater>
       ...
       <div class="wzxx">
           <asp:Repeater ID="rptgx" runat="server">
               <HeaderTemplate><ul></HeaderTemplate>
               <ItemTemplate>
                   <li><span class="zitixs">下一篇</span>：<asp:HyperLink ID="hylss1" runat="server" Text='<%# ziduanjq.Jq_xwbtxs(Eval("msTitle").ToString()) %>' NavigateUrl='<%# Eval("msId","sy_ms_content.aspx?ms_id={0}") %>'>
                       </asp:HyperLink><br/></li>
               </ItemTemplate>
               <FooterTemplate></ul></FooterTemplate>
           </asp:Repeater>
   ...
   <div class="right">
       ...
       <div class="r_con">
           <asp:Repeater ID="rptgy1" runat="server">
             ...
             <ItemTemplate>
                 <li>
                     <asp:HyperLink ID="hyplk1" runat="server" Text='<%# ziduanjq.Jq_xwbt2(Eval("msTitle").ToString()) %>' NavigateUrl='<%# Eval("msId","sy_ms_content.aspx?ms_id={0}") %>'></asp:HyperLink></li>
             </ItemTemplate>
             <FooterTemplate></ul></FooterTemplate>
           </asp:Repeater>
       </div>
       ...
       <div class="r_con">
           <asp:Repeater ID="rptgy2" runat="server">
             ...
             <ItemTemplate>
                 <li>
                     <asp:HyperLink ID="hyplk2" runat="server" Text='<%# ziduanjq.Jq_xwbt2(Eval("msTitle").ToString()) %>' NavigateUrl='<%# Eval("msId","sy_ms_content.aspx?ms_id={0}") %>'></asp:HyperLink></li>
             </ItemTemplate>
             <FooterTemplate></ul></FooterTemplate>
           </asp:Repeater>
       </div>
