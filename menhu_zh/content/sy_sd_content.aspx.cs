...
public partial class content_sy_sd_content : System.Web.UI.Page
{
    ...
    {
        string gyid = Request["sd_id"].ToString();
        int id = Convert.ToInt32(gyid);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sdId(id).DefaultView;
    ...
    {
        string sql = "select top 5 sdid,sdtitle from sy_study order by sdtime desc";
        PagedDataSource pds = new PagedDataSource();
    ...
    {
        string sql = "select top 5 sdid,sdtitle from sy_study order by sdpl desc";
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_news(sql).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 5;
    ...
    {
        string id = Request["sd_id"].ToString();
        int gyid = Convert.ToInt32(id);
        rptpl.DataSource = sybll.select_sdPl(gyid);
        rptpl.DataBind();
    }
    protected void btnfb_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (sybll.Insert_sdpl(id1, name, SqlInsertEncode(con)) > 0)
            {
                sybll.Update_addsdpl(id1);
    ...
    private void binddatagys() //上一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sdxsId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    private void binddatagyx() //下一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sdxxId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    protected void btnserach_Click(object sender, EventArgs e)
    {
        ...
        if (txtserach.Text != "")
        {
            string sql = "select top 1 * from sy_study where sdtitle like '%'+@serach+'%' or sdcon like '%'+@serach+'%'";
