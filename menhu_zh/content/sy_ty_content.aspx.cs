...
public partial class content_sy_ty_content : System.Web.UI.Page
{
    ...
    {
        string gyid = Request["ty_id"].ToString();
        int id = Convert.ToInt32(gyid);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_tyId(id).DefaultView;
    ...
    {
        string sql = "select top 5 tyid,tytitle from sy_tiyu order by tytime desc";
        PagedDataSource pds = new PagedDataSource();
    ...
    {
        string sql = "select top 5 tyid,tytitle from sy_tiyu order by typl desc";
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_news(sql).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 5;
    ...
    {
        string id = Request["ty_id"].ToString();
        int gyid = Convert.ToInt32(id);
        rptpl.DataSource = sybll.select_tyPl(gyid);
        rptpl.DataBind();
    }
    protected void btnfb_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (sybll.Insert_typl(id1, name, SqlInsertEncode(con)) > 0)
            {
                sybll.Update_addtypl(id1);
    ...
    private void binddatagys() //上一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_tyxsId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    private void binddatagyx() //下一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_tyxxId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    protected void btnserach_Click(object sender, EventArgs e)
    {
        ...
        if (txtserach.Text != "")
        {
            string sql = "select top 1 * from sy_tiyu where tytitle like '%'+@serach+'%' or tycon like '%'+@serach+'%'";
