...
public partial class content_sy_kj_content : System.Web.UI.Page
{
    ...
    {
        string gyid = Request["kj_id"].ToString();
        int id = Convert.ToInt32(gyid);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_kjId(id).DefaultView;
    ...
    {
        string sql = "select top 5 kjid,kjtitle from sy_keji order by kjtime desc";
        PagedDataSource pds = new PagedDataSource();
    ...
    {
        string sql = "select top 5 kjid,kjtitle from sy_keji order by kjpl desc";
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_news(sql).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 5;
    ...
    {
        string id = Request["kj_id"].ToString();
        int gyid = Convert.ToInt32(id);
        rptpl.DataSource = sybll.select_kjPl(gyid);
        rptpl.DataBind();
    }
    protected void btnfb_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (sybll.Insert_kjpl(id1, name, SqlInsertEncode(con)) > 0)
            {
                sybll.Update_addkjpl(id1);
    ...
    private void binddatagys() //上一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_kjxsId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    private void binddatagyx() //下一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_kjxxId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    protected void btnserach_Click(object sender, EventArgs e)
    {
        ...
        if (txtserach.Text != "")
        {
            string sql = "select top 1 * from sy_keji where kjtitle like '%'+@serach+'%' or kjcon like '%'+@serach+'%'";
