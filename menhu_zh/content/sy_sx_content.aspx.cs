...
public partial class content_sy_sx_content : System.Web.UI.Page
{
    ...
    {
        string gyid = Request["sx_id"].ToString();
        int id = Convert.ToInt32(gyid);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sxId(id).DefaultView;
    ...
    {
        string sql = "select top 5 sxid,sxtitle from sy_shix order by sxtime desc";
        PagedDataSource pds = new PagedDataSource();
    ...
    {
        string sql = "select top 5 sxid,sxtitle from sy_shix order by sxpl desc";
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_news(sql).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 5;
    ...
    {
        string id = Request["sx_id"].ToString();
        int gyid = Convert.ToInt32(id);
        rptpl.DataSource = sybll.select_sxPl(gyid);
        rptpl.DataBind();
    }
    protected void btnfb_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (sybll.Insert_sxpl(id1, name, SqlInsertEncode(con)) > 0)
            {
                sybll.Update_addsxpl(id1);
    ...
    private void binddatagys() //上一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sxxsId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    private void binddatagyx() //下一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_sxxxId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    protected void btnserach_Click(object sender, EventArgs e)
    {
        ...
        if (txtserach.Text != "")
        {
            string sql = "select top 1 * from sy_shix where sxtitle like '%'+@serach+'%' or sxcon like '%'+@serach+'%'";
