...
public partial class content_sy_qc_content : System.Web.UI.Page
{
    ...
    {
        string gyid = Request["qc_id"].ToString();
        int id = Convert.ToInt32(gyid);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_qcId(id).DefaultView;
    ...
    {
        string sql = "select top 5 qcid,qctitle from sy_qiche order by qctime desc";
        PagedDataSource pds = new PagedDataSource();
    ...
    {
        string sql = "select top 5 qcid,qctitle from sy_qiche order by qcpl desc";
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_news(sql).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 5;
    ...
    {
        string id = Request["qc_id"].ToString();
        int gyid = Convert.ToInt32(id);
        rptpl.DataSource = sybll.select_qcPl(gyid);
        rptpl.DataBind();
    }
    protected void btnfb_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (sybll.Insert_qcpl(id1, name, SqlInsertEncode(con)) > 0)
            {
                sybll.Update_addqcpl(id1);
    ...
    private void binddatagys() //上一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_qcxsId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    private void binddatagyx() //下一篇
    {
        ...
        int gyid = Convert.ToInt32(id);
        PagedDataSource pds = new PagedDataSource();
        pds.DataSource = sybll.select_qcxxId(gyid).DefaultView;
        pds.AllowPaging = true;
        pds.PageSize = 1;
    ...
    protected void btnserach_Click(object sender, EventArgs e)
    {
        ...
        if (txtserach.Text != "")
        {
            string sql = "select top 1 * from sy_qiche where qctitle like '%'+@serach+'%' or qccon like '%'+@serach+'%'";
