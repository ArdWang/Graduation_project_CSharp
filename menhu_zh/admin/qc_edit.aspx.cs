...
public partial class admin_qc_edit : System.Web.UI.Page
{
    ...
    {
        string qcId = grdqc.DataKeys[e.RowIndex].Value.ToString();
        int id = Convert.ToInt32(qcId);
        if (sybll.Delete_qc(id) > 0)
        {
            sybll.Delete_plqc_del(id);
    ...
    protected void grdqc_RowEditing(object sender, GridViewEditEventArgs e)
    {
        grdqc.EditIndex = e.NewEditIndex;
        binddataqc();
    }
    protected void grdqc_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        string qcId = grdqc.DataKeys[e.RowIndex].Value.ToString();
        int id = Convert.ToInt32(qcId);
        string qcbt = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[1].Controls[0].FindControl("txtqcbt"))).Text.ToString();
        string qccon = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[2].Controls[0].FindControl("txtqccon"))).Text.ToString();
        string qcprice = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[3].Controls[0].FindControl("txtqcprice"))).Text.ToString();
        string qcimg = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[4].Controls[0].FindControl("txtimg"))).Text.ToString();
        string qcman = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[5].Controls[0].FindControl("txtqcman"))).Text.ToString();
        string qctime = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[6].Controls[0].FindControl("txtqctime"))).Text.ToString();
        string qcpl = ((TextBox)(grdqc.Rows[e.RowIndex].Cells[7].Controls[0].FindControl("txtqcpl"))).Text.ToString();
        int pl = Convert.ToInt32(qcpl);
        FileUpload fuimg = ((FileUpload)(grdqc.Rows[e.RowIndex].Cells[3].Controls[0].FindControl("fuimg")));
        /*判断图片代码*/
        string filePath = qcimg;
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                filePath = "../images/qc/" + fuimg.FileName;
        ...
        if (sybll.Update_qc(id, qcbt, qccon,qcprice, filePath, qcman, qctime, pl) > 0)
        {
    ...
    protected void grdqc_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
    {
        grdqc.EditIndex = -1;
        binddataqc();
    }
