...
public partial class admin_ly_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string lybt = txtlybt.Text.Trim();
            string lycon = content1.Value.Trim();
            string price = txtprice.Text.Trim();
            string lyImg = gettup();
            string fbman = txtfbman.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_ly(lybt, lycon, price, lyImg, fbman, time) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/ly/" + fuimg.FileName;
