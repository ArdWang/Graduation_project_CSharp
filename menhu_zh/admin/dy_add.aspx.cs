...
public partial class admin_dy_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string dybt = txtdybt.Text.Trim();
            string dycon = content1.Value.Trim();
            string dyImg = gettup();
            string dyyy = txtyy.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_dy(dybt, dycon, dyImg,dyyy, time) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/dy/" + fuimg.FileName;
