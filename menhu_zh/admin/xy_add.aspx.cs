...
public partial class admin_xy_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string newsbt = txtxybt.Text.Trim();
            string newscon = content1.Value.Trim();
            string newsImg = gettup();
            string fbman = txtfbman.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_xy(newsbt, newscon, newsImg, fbman, time) > 0)
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "", "alert('发布新闻成功!');", true);
            }
            else
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "", "alert('发布新闻失败!');", true);
        }
    }
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/xy/" + fuimg.FileName;
