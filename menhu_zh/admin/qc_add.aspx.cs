...
public partial class admin_qc_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string qcbt = txtqcbt.Text.Trim();
            string qccon = content1.Value.Trim();
            string price = txtprice.Text.Trim();
            string qcImg = gettup();
            string fbman = txtfbman.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_qc(qcbt,qccon,price, qcImg, fbman, time) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/qc/" + fuimg.FileName;
