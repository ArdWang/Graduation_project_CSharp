...
public partial class admin_plty_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_ty order by pltyId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_plty(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["tyID"].ToString());
        if (sybll.Delete_plty(id) > 0)
        {
            sybll.Update_downtypl(id2);
