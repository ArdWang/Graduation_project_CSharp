...
public partial class admin_plkj_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_kj order by plkjId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_plkj(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["kjID"].ToString());
        if (sybll.Delete_plkj(id) > 0)
        {
            sybll.Update_downkjpl(id2);
