...
public partial class admin_yl_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string ylbt = txtylbt.Text.Trim();
            string ylcon = content1.Value.Trim();
            string ylImg = gettup();
            string fbman = txtfbman.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_yl(ylbt, ylcon, ylImg, fbman, time) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/yl/" + fuimg.FileName;
