...
public partial class admin_plsx_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_sx order by plsxId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_plsx(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["sxID"].ToString());
        if (sybll.Delete_plsx(id) > 0)
        {
            sybll.Update_downsxpl(id2);
