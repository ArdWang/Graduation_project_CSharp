...
public partial class admin_pljk_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_jk order by pljkId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_pljk(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["jkID"].ToString());
        if (sybll.Delete_pljk(id) > 0)
        {
            sybll.Update_downjkpl(id2);
