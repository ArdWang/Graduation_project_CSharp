...
public partial class admin_sp_add : System.Web.UI.Page
{
    ...
    {
        string sql="select * from sy_spfl";
        sydb.bind_D(ddlfl, sql);
    }
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string spbt = txtspbt.Text.Trim();
            string fl = ddlfl.SelectedItem.Text;
            int flid = Convert.ToInt32(fl);
            string price = txtprice.Text.Trim();
            string spImg = gettup();
            string fbman = txtfbman.Text.Trim();
            if (sybll.Insert_sp(flid,spbt,price, spImg, fbman) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/sp/" + fuimg.FileName;
