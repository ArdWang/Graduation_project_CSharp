...
public partial class admin_plms_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_ms order by plmsId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_plms(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["msID"].ToString());
        if (sybll.Delete_plms(id) > 0)
        {
            sybll.Update_downmspl(id2);
