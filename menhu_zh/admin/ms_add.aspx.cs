...
public partial class admin_ms_add : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            string msbt = txtmsbt.Text.Trim();
            string mscon = content1.Value.Trim();
            string msImg = gettup();
            string fbman = txtfbman.Text.Trim();
            string time = txttime.Text.Trim();
            if (sybll.Insert_ms(msbt, mscon, msImg, fbman, time) > 0)
            {
    ...
    private string gettup()
    {
        if (fuimg.HasFile)//判断是否有文件
        {
            ...
            if (CheckFileType(fuimg.FileName))//检查上传文件的类型
            {
                ...
                string filePath = "../images/ms/" + fuimg.FileName;
