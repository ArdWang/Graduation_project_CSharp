...
public partial class admin_plly_edit : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from pl_ly order by pllyId desc";
    ...
    protected void grdgypl_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (sybll.Update_plly(id, plname, plcon) > 0)
        {
    ...
    protected void grdgypl_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        int id2 = Convert.ToInt32(dt.Rows[0]["lyID"].ToString());
        if (sybll.Delete_plly(id) > 0)
        {
            sybll.Update_downlypl(id2);
